 ...
 */
	...
	#pragma omp parallel for default(none) private(i) firstprivate(rows, columns) shared(culture)
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		*/
		...
		#pragma omp parallel for schedule(guided) firstprivate(num_cells) reduction(max:auxiliar) private(i) reduction(+:num_cells_alive, step_dead_cells) shared(food_to_share, culture_cells, culture, cells, columns, rows)
		/* 4.3. Cell movements */
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				cells[i].age ++;
