// User: g215@83.56.97.72 
// ExecutionRequest[P:'extinguishing-04.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 13 2019 19:04:48
 ...
 */
	...
	#pragma omp parallel/*dynamic(64)*/ for //collapse(2)
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ )
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		#pragma omp parallel for private(i) reduction(+:num_deactivated)
		for( i=0; i<num_focal; i++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			#pragma omp /*parallel*/ for private(i,j) /*collapse(2)*/ /*schedule(static)*/ nowait
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ )
			...
			#pragma omp /*parallel*/ for /*collapse(2)*/ reduction(max:global_residual) nowait
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ )
		...
		/* 4.3. Move teams */
		#pragma omp parallel for /*private(t)*/default(none) firstprivate(num_focal , focal, num_teams) shared(teams) private(t)
		for( t=0; t<num_teams; t++ ) {
		...
		/* 4.4. Team actions */
		//#pragma omp parallel for //ERROR PEQUE
