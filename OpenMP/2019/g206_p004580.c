// User: g206@90.161.132.63 
// ExecutionRequest[P:'seremlliures.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 17:56:46
...
#pragma omp parallel for	
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		int step;
		int test = num_deactivated == num_focal;
		for( step=0; step<10; step++ )	{
			...
			if (step == 0 && test){
                ...
                #pragma omp parallel  for  reduction(max:global_residual) /** A veces tarda mucho por alguna razón **/ 
                for( i=1; i<rows-1; i++ )
                    for( j=1; j<columns-1; j++ ) 
		...
		//No varía nada hacer el && o hacer dos ifs anidados
