// User: g210@157.88.125.192 
// ExecutionRequest[P:'PedroPresidente.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 13:52:56
...
float *pointer;
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		#pragma omp parallel for shared (focal), private(i) reduction(+:num_deactivated) // Parece q sube tiempo :C
		for( i=0; i<num_focal; i++ ) {
...
;
				surface[ focal[i].x * columns + focal[i].y ]= focal[i].heat;
			}


			/* 4.2.2. Copy values of the surface in ancillary structure (Skip borders) */  // sobra
			//#pragma omp parallel for shared(surfaceCopy,surface), private(j)
			...
			/* 4.2.3. Update surface values (skip borders)  // first private row columns y todo en uno mismo.. :C*/
			#pragma omp parallel for private(j)  reduction(max:global_residual) shared(surfaceCopy,surface)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ){
					*(surface+(i * columns + j))  = ( 
						...
						*(surfaceCopy+(i * columns + (j+1))) ) * 0.25;

					if (  *(surface+(i * columns + j)) - *(surfaceCopy+(i * columns + j)) > global_residual){
						global_residual =  *(surface+(i * columns + j)) - *(surfaceCopy+(i * columns + j)) ;
					}
				}
		...
		/* 4.4. Team actions */
		#pragma omp parallel for private(t,i,j) shared (surface)
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				...
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j)  <= radius*radius ) {
						#pragma omp atomic
						 *(surface+(i * columns + j)) = *(surface+( i * columns + j)) * (0.75); // Team efficiency factor
					}
				}
			}
		}
