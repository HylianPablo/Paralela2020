 ...
 *
 */
//probar a quitar los private i y el firstprivate
	...
	int* auxCopia=(int*)malloc(sizeof(int)*(size_t)rows*(size_t)columns);
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			#pragma omp parallel for private(i), firstprivate(num_focal), shared(focal,surface)
			for( i=0; i<num_focal; i++ ) {
...
/*
			#pragma omp parallel for private(i,j),firstprivate(rows,columns), shared(surfaceCopy,surface)
			for( i=1; i<rows-1; i++ )
...
*/
			auxCopia=surface;
			surface=surfaceCopy;
			surfaceCopy=auxCopia;
			/* 4.2.3. Update surface values (skip borders) */
			#pragma omp parallel for private(i,j), firstprivate(rows,columns), shared(surfaceCopy,surface),reduction(max:global_residual)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ){
			...
			//Aqui se puede eliminar los bucles for y luego se realiza arriba un reduction parallel con max global residual y con eso se consigue bajar un monton de tiempo
			//Metiendo el if dentro del bucle de arriba
				}
		}
		...
		/* 4.3. Move teams */
		#pragma omp parallel for default(none) private(t,j), shared(teams,focal), firstprivate(num_teams,num_focal)
		for( t=0; t<num_teams; t++ ){
		...
		for( t=0; t<num_teams; t++ ) {
			...
			int x = teams[t].x;
			int y = teams[t].y;
			if ( target != -1 && focal[target].x == x && focal[target].y == y 
