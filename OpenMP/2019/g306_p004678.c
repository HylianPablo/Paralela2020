...
// ExecutionRequest[P:'TheGame.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 18:56:29
...
//	double ttempo4_4;		/*-----*/
	...
	// 1
	#pragma omp parallel for private(j) //nowait
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ )
	...
	int *aux = (float *)malloc( sizeof(float) * (size_t)rows * (size_t)columns );
...
//		ttempo4_1 = cp_Wtime();			/*-----*/
		...
		// 2.1
		#pragma omp parallel for default(none) firstprivate(iter, num_focal) shared(focal) reduction(+:num_deactivated)
		for( i=0; i<num_focal; i++ ) {
...
//			ttempo4_2_1 = cp_Wtime();					/*-----*/

			// 2.2.1
			#pragma omp parallel for default(none) firstprivate(focal, num_focal, columns, rows) shared(surface)
			for( i=0; i<num_focal; i++ ) {
				if ( focal[i].active != 1 ) continue;
				accessMat( surface, focal[i].x, focal[i].y ) = focal[i].heat;
			}

//			t4_2_1 += (cp_Wtime() - ttempo4_2_1);     /*-----*/

//			ttempo4_2_2 = cp_Wtime();					/*-----*/

			aux=surface;
		    surface=surfaceCopy;
    		surfaceCopy=aux;

//			t4_2_2 += (cp_Wtime() - ttempo4_2_2);     /*-----*/

//			ttempo4_2_3 = cp_Wtime();					/*-----*/
			...
			float temp4;
			// 2.2.3
			#pragma omp parallel default(none) private(j) firstprivate(rows,columns, tempT, temp1, temp2, temp3, temp4) shared(surface, surfaceCopy, global_residual)
			{
			#pragma omp for nowait
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) {
					temp1 = accessMat( surfaceCopy, i-1, j );
					temp2 = accessMat( surfaceCopy, i+1, j );
					temp3 = accessMat( surfaceCopy, i, j-1 );
					temp4 = accessMat( surfaceCopy, i, j+1 );
					tempT = (temp1 + temp2 + temp3 + temp4) / 4;
					accessMat(surface, i, j) = tempT;
				}

			// 2.2.4
			
			#pragma omp for reduction(max:global_residual) nowait
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) {
...
//		ttempo4_3 = cp_Wtime();				/*-----*/
		...
		//#pragma omp parallel
		//{
		#pragma omp parallel for default(none) private(j) shared(teams) firstprivate(focal, num_focal, num_teams)
		for( t=0; t<num_teams; t++ ) {
			...
			int comp1 = (focal[target].x < teams[t].x);
			int comp2 = (focal[target].x > teams[t].x);
			int comp3 = (focal[target].y < teams[t].y);
			int comp4 = (focal[target].y > teams[t].y);
			...
			switch (teams[t].type) { 
				case 1:
					...
					if ( comp1 ) teams[t].x--;
					else if ( comp2 ) teams[t].x++;
					if ( comp3 ) teams[t].y--;
					else if ( comp4 ) teams[t].y++;
					break;
				...
				default:
					...
					else if ( comp3 ) teams[t].y--;
					else if ( comp4 ) teams[t].y++;
					break;
			}
		}

//		t4_3 += (cp_Wtime() - ttempo4_3);			/*-----*/

//		ttempo4_4 = cp_Wtime();				/*-----*/

		// 2.4
		#pragma omp parallel for default(none) private(i, j) firstprivate(num_teams, focal, teams, columns, rows) shared(surface)
		for( t=0; t<num_teams; t++ ) {
...
//	printf("Part 1 Time: %lf\n", t4_1 );			/*-----*/
//	printf("Part 2 Time: %lf\n", t4_2 );			/*-----*/
//	printf("Part 2.1 Time: %lf\n", t4_2_1 );		/*-----*/
//	printf("Part 2.2 Time: %lf\n", t4_2_2 );		/*-----*/
//	printf("Part 2.3 Time: %lf\n", t4_2_3 );		/*-----*/
//	printf("Part 3 Time: %lf\n", t4_3 );			/*-----*/
//	printf("Part 4 Time: %lf\n", t4_4 );			/*-----*/
