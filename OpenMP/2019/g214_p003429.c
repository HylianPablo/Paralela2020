...
// ExecutionRequest[P:'dreaming.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 01:06:06
 ...
 */
	...
	#pragma omp parallel for default(none) shared(surface) private(i,j) firstprivate(rows, columns)
	for(i=0; i<rows; i++ )
		for(j=0; j<columns; j++ )
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		if(!flag_deactivated){
			...
			if(num_deactivated == num_focal) flag_deactivated = 1;
		}else num_deactivated = num_focal;
...
//-------------------------------------
		...
		temp = surfaceCopy;
		surfaceCopy = (float*) surface;

		/* 4.2.3. Update surface values (skip borders) */
		#pragma omp parallel for default(none) private(i,j) firstprivate(rows,columns) shared(surfaceCopy,temp)
		for( i=1; i<(rows-1); i++)			
		...
		if(flag_deactivated)
			#pragma omp parallel for default(none) private(i,j) firstprivate(rows,columns) shared(surfaceCopy, surface) reduction(max:global_residual)
			for( i=1; i<(rows-1); i++ )
				for( j=1; j<(columns-1); j++ )
					global_residual = global_residual > accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) ? global_residual : accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j );
...
//-----------------------------------
		...
		if( flag_deactivated && global_residual < THRESHOLD ) flag_stability = 1;
		
		/* 4.3. Move teams */
		#pragma omp parallel private(t, i, j) default(shared)
		{
		#pragma omp for
		for( t=0; t<num_teams; t++ ) {
			...
			if(!flag_deactivated)
				for( j=0; j<num_focal; j++ ) {
					...
					float dx = focal[j].x - a;
					float dy = focal[j].y - b;
			...
			int focx = focal[target].x;
			int focy = focal[target].y;
			/* 4.3.4. Move in the focal point direction */
			if ( teams[t].type == 1 ) { 
				// Type 1: Can move in diagonal
				if ( focx < teams[t].x ) teams[t].x--;
				if ( focx > teams[t].x ) teams[t].x++;
				if ( focy < teams[t].y ) teams[t].y--;
				if ( focy > teams[t].y ) teams[t].y++;
			}
			else if ( teams[t].type == 2 ) { 
				// Type 2: First in horizontal direction, then in vertical direction
				if ( focy < teams[t].y ) teams[t].y--;
				else if ( focy > teams[t].y ) teams[t].y++;
				else if ( focx < teams[t].x ) teams[t].x--;
				else if ( focx > teams[t].x ) teams[t].x++;
			}
			else {
