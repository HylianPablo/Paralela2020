...
// ExecutionRequest[P:'27eurovision27.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 17:57:49
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		int step;
		#pragma omp parallel private(step,i,j)
		{
		...
		//TO_BOTTOM if( num_deactivated == num_focal && global_residual < THRESHOLD ) flag_stability = 1;

		/* 4.3. Move teams */
		#pragma omp for private(t,j)
		for( t=0; t<num_teams; t++ ) {
