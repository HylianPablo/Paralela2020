// User: g213@157.88.125.134 
// ExecutionRequest[P:'extinguishingVilla.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 13 2019 15:41:58
 ...
 */
	...
	#pragma omp schedule(dynamic, 64)
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		/* 4.3. Move teams */
		#pragma omp parallel for firstprivate(num_focal, focal, num_teams) shared(teams)
		for( t=0; t<num_teams; t++ ) {
		...
		/* 4.4. Team actions */
		#pragma omp parallel for default(none) firstprivate(rows, columns, focal, num_teams, teams) private(i,j) shared(surface)
		for( t=0; t<num_teams; t++ ) {
