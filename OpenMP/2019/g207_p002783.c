 ...
 */
	...
	for( iter=0; iter<max_iter && !flag_stability; iter++ ) {
		...
		//int num_deactivated = 0;//empezamos con 0 focos desactivados cada iteracion
		
		if(num_deactivated!=num_focal)//mencionar a duero en la memoria
		#pragma omp parallel for reduction(+:num_deactivated)
		for( i=0; i<num_focal; i++ ) {
			//si momento iniciar foco coincide con iteracion actual, se activa
			if ( focal[i].start == iter ) focal[i].active = 1;
		...
		for( step=0; step<10; step++ )	{
			#pragma omp parallel shared(surface,surfaceCopy)
			{
			...
			if(num_deactivated==num_focal)//mencionar a duero en la memoria
			#pragma omp for /*collapse(2)*/ reduction(max:global_residual) 
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) {
					float res_diff=accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j );
					if ( res_diff > global_residual  )global_residual = res_diff;
				}
			}
		...
		#pragma omp parallel default(none) shared(num_teams,columns,rows,num_focal,surface,num_deactivated) private(j,i,t) firstprivate(focal,teams)
		...
		//}
		/* 4.4. Team actions */
		#pragma omp for reduction(+:num_deactivated)
		for( t=0; t<num_teams; t++ ) {
			...
			//printf("deactivated: %d",num_deactivated);
 ...
 */
	...
	/*
	for (i=0; i<num_teams; i++)
		printf(" T: %d %d", teams[i].x, teams[i].y );
	*/
	for (i=0; i<num_focal; i++)
