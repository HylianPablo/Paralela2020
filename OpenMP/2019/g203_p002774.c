// User: g203@157.88.139.133 
// ExecutionRequest[P:'TeMiroYteParalelizo.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 14 2019 17:56:31
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
        ...
        #pragma omp parallel for schedule(static,8) reduction(+:num_deactivated) // Esto baja algo
		for( i=0; i<num_focal; i++ ) {
      ...
      {
		for( step=0; step<10; step++ )	{ //si desenrrollas ganas algo 
         ...
         //   #pragma omp  for  //collapse(1)
            for( i=0; i<num_focal; i++ ) {
				...
				int x = focal[i].x;
				int y = focal[i].y;
				surface[(x*columns)+y] = focal[i].heat;
			}
		////////////////////////////////////////////////////////////	arr[ (exp1) * columns + (exp2) ]    surfaceCopy[ (i*columns)+ j ] surface[(i*columns)+ j]
			/* 4.2.2. Copy values of the surface in ancillary structure (Skip borders) */
            //#pragma omp parallel for private(j) collapse(1)
          //    #pragma omp  parallel for   private(j) collapse(1)
		//	for( i=1; i<rows-1; i++ )
		//		for( j=1; j<columns-1; j++ )
			//		 surfaceCopy[ (i*columns)+ j ] = surface[(i*columns)+ j] ;
			#pragma omp single
			{
			puntero=surface;
			surface=surfaceCopy;
            ...
            //#pragma omp parallel for private(i,j) 
              #pragma omp  for  private(j) reduction(max:global_residual) 
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ )
				{
					surface[(i*columns)+ j] = ( surfaceCopy[ ((i-1)*columns)+ j ]+surfaceCopy[ ((i+1)*columns)+ j ]+surfaceCopy[ (i*columns)+ j-1 ]+surfaceCopy[ (i*columns)+ j+1 ]
						) / 4;
					if ( fabs( surface[(i*columns)+ j] - surfaceCopy[ (i*columns)+ j ]> global_residual ) ) {
						global_residual = fabs( surface[(i*columns)+ j] - surfaceCopy[ (i*columns)+ j ] );
					}
				}
}
			/* 4.2.4. Compute the maximum residual difference (absolute value) */
          //  #pragma omp parallel for private(j) reduction(max:global_residual)
        //   #pragma omp parallel 
      // {
       //     #pragma omp for  private(j) collapse(1)
		//	for( i=1; i<rows-1; i++ )
		//		for( j=1; j<columns-1; j++ ) 
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
              //  #pragma omp parallel for 
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( distance <= radius ) {
                        #pragma omp atomic
						surface[(i*columns)+ j] = surface[(i*columns)+ j] * ( 1 - 0.25 ); // Team efficiency factor
					}
                    //fin ciritcal 
				}
			}
