 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		//#pragma omp parallel for private(i),shared(focal) reduction(+:num_deactivated)
		for( i=0; i<num_focal; i++ ) {
		...
		for( t=0; t<num_teams; t++ ) {
			...
			#pragma omp parallel for private(j) reduction(min:distance)
		...
		#pragma omp parallel for default(none) firstprivate (num_teams, rows, columns, teams, focal ) shared (surface) private (i, j)
		for( t=0; t<num_teams; t++ ) {
