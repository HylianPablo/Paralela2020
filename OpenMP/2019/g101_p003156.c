...
// ExecutionRequest[P:'VoyVolando.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 14 2019 21:39:16
 ...
 */
	...
	#pragma omp parallel for private(i,j) shared(surface) 
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ )
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		//int step;
			...
			#pragma omp parallel for private(i,j) firstprivate(rows,columns) reduction(max:global_residual)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ )
				{
					...
					if ( num_deactivated == num_focal && ( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) > global_residual ) ) {
					...
					if ( num_deactivated == num_focal && ( accessMat( surface, i, j+1 ) - accessMat( surfaceCopy, i, j+1 ) > global_residual ) ) {
			...
			#pragma omp parallel for private(i,j) firstprivate(rows,columns) 
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ )
				{
					accessMat( surface, i, j ) = (
		...
		/* 4.3. Move teams */
		//#pragma omp parallel for default(none) shared(teams,focal,num_focal) private(t,j) firstprivate(num_teams,target,distance,local_distance)
		#pragma omp parallel for shared(teams,focal) private(t,j) 
		for( t=0; t<num_teams; t++ ) {
			...
			float tx = teams[t].x;
			float ty = teams[t].y;
			#pragma omp parallel for shared(focal,teams) private(j) firstprivate(num_focal)
			for( j=0; j<num_focal; j++ ) {
				if ( focal[j].active != 1 ) continue; // Skip non-active focal points
				//float dx = focal[j].x - teams[t].x;
				//float dy = focal[j].y - teams[t].y;
				float local_distance  = ( (focal[j].x - tx)*(focal[j].x - tx) + (focal[j].y - ty)*(focal[j].y - ty) );
		...
		#pragma omp parallel for private(t,i,j)  firstprivate(num_teams)
		for( t=0; t<num_teams; t++ ) {
			/* 4.4.1. Deactivate the target focal point when it is reached */
			int target = teams[t].target;
			if ( target != -1 && focal[target].active == 1  && focal[target].x == teams[t].x && focal[target].y == teams[t].y
				)
