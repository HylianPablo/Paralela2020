// User: g104@62.42.137.146 
// ExecutionRequest[P:'ZA_WARUDO.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 19:00:44
...
#pragma omp parallel default(none) private(i, j) shared(surface, rows, columns)
	{
#pragma omp for
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ )
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		int step;
#pragma omp parallel default(none) private(i, j, step) shared(surface, surfaceCopy, rows, columns, focal, num_focal, global_residual)
		{
...
#pragma omp for reduction (max:global_residual)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) 
		...
		/* 4.3. Move teams */
#pragma omp parallel default(none) private(t, j) shared(num_teams, num_focal, focal, teams)
		{
#pragma omp for
		for( t=0; t<num_teams; t++ ) {
		...
		/* 4.4. Team actions */
#pragma omp parallel default(none) private(t, i, j) shared(num_teams, teams, focal, rows, columns, surface)
		{
#pragma omp for
		for( t=0; t<num_teams; t++ ) {
