 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		float global_residual = 0.0f;
			...
			//#pragma omp parallel 
			//{ 
				//#pragma omp for private(i,j) firstprivate(rows)
				//for( i=1; i<rows-1; i++ )
			...
			//}//ACABA AQUI EL PRAGMA OMP PARALLEL
		...
		if( global_residual < THRESHOLD && num_focal == num_deactivated) flag_stability = 1;
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				int valor=(teams[t].x - i)*(teams[t].x - i);
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( ( valor + (teams[t].y - j)*(teams[t].y - j) ) <= (radius*radius)) {
						#pragma omp atomic
