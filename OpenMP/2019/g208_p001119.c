 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		/* 4.4. Team actions */
		#pragma omp parallel for default(none) firstprivate(rows, columns, focal, num_teams, teams) shared(surface) private (i,j)
		for( t=0; t<num_teams; t++ ) {
