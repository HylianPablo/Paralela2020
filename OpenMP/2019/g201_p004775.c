...
// ExecutionRequest[P:'seq.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 19:41:37
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		#pragma omp parallel shared(surface,teams) 
		{
...
{	
	...
	#pragma omp for private(j) reduction(max:global_residual) nowait 
		...
		/* 4.3. Move teams */
		//taux=cp_Wtime();
		#pragma omp for private(j) schedule(dynamic,64)
		for( t=0; t<num_teams; t++ ) {
	...
	/*
		printf("tiempo bucle 4.2.4 %f \n",t1);
		printf("tiempo bucle 4.2.3 %f \n",t2);
		printf("tiempo bucle 4.2.2 %f \n",t3);
		printf("tiempo mov team %f \n",t4);
		printf("tiempo acc team %f \n",t5);
/*
 *
