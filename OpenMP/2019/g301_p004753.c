// User: g301@157.88.125.192 
// ExecutionRequest[P:'extinguishing.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 19:30:45
 ...
 */
  ...
  float* matAux = (float *)malloc( sizeof(float) * (size_t)rows * (size_t)columns );
	switch( rows*columns%2 ){
		case 1:
				#pragma omp parallel for private(i,j) firstprivate(rows, columns) shared(surface)
				for( i=0; i<rows; i++ )
					for( j=0; j<columns; j++ ){
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		//#pragma omp parallel for default(none) shared(focal, num_deactivated) firstprivate(iter, num_focal) private(i)
		for( i=0; i<num_focal; i++ ) {
			if ( focal[i].start == iter ) {
				focal[i].active = 1;
				//Eliminada variable first_activation, ningun uso
			}
		...
		for( step=0; step<10; step++ )	{
			...
			#pragma omp parallel for firstprivate(num_focal) private(i) shared(focal, surface)//----------------------------------------TODO
			for( i=0; i<num_focal; i++ ) {
				if ( focal[i].active != 1 ) continue;
				//Creacion de variables eliminada
				accessMat( surface, focal[i].x, focal[i].y ) = focal[i].heat;
			}

			/* 4.2.2. Copy values of the surface in ancillary structure (Skip borders) */
			// #pragma omp parallel for firstprivate(rows, columns) shared(surfaceCopy, surface) private(i,j)
			// for( i=1; i<rows-1; i++ )
			...
			#pragma omp parallel for firstprivate(columns, rows), shared (surface, surfaceCopy), private(i,j), reduction(max:global_residual)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ){
			...
			//Esta parte del codigo ha sido incluida en el bucle anterior
		}
		...
		/* 4.3. Move teams */
		#pragma omp parallel for default(none) firstprivate(num_teams, num_focal) private(t, j) shared(focal, teams)
		for( t=0; t<num_teams; t++ ) {
			...
			/* 4.3.4. Move in the focal point direction. Modificado de un if*/
			switch( teams[t].type ){
				case 1:	// Type 1: Can move in diagonal
								if ( focal[target].x < teams[t].x ) teams[t].x--;
								if ( focal[target].x > teams[t].x ) teams[t].x++;
								if ( focal[target].y < teams[t].y ) teams[t].y--;
								if ( focal[target].y > teams[t].y ) teams[t].y++;
								break;
				case 2:	// Type 2: First in horizontal direction, then in vertical direction
								if ( focal[target].y < teams[t].y ) teams[t].y--;
								else if ( focal[target].y > teams[t].y ) teams[t].y++;
								else if ( focal[target].x < teams[t].x ) teams[t].x--;
								else if ( focal[target].x > teams[t].x ) teams[t].x++;
								break;
				case 3:	// Type 3: First in vertical direction, then in horizontal direction
								if ( focal[target].x < teams[t].x ) teams[t].x--;
								else if ( focal[target].x > teams[t].x ) teams[t].x++;
								else if ( focal[target].y < teams[t].y ) teams[t].y--;
								else if ( focal[target].y > teams[t].y ) teams[t].y++;
		...
		/* 4.4. Team actions */
		#pragma omp parallel for default(none) firstprivate(teams, num_teams, rows, columns, focal) shared(surface) private(i,j)
		for( t=0; t<num_teams; t++ ) {
			/* 4.4.1. Deactivate the target focal point when it is reached */
			int target = teams[t].target;
			int tx = teams[t].x;
			int ty = teams[t].y;
			if ( target != -1 && focal[target].x == tx && focal[target].y == ty
			...
			for( i=tx-radius; i<=tx+radius; i++ ) {
				for( j=ty-radius; j<=ty+radius; j++ ) {
