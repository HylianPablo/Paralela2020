...
// ExecutionRequest[P:'XD.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 19:38:42
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		int step;
			...
			#pragma omp parallel for reduction(max:global_residual),private(j),firstprivate(rows,columns)
			for( i=1; i<rows-1; i++ )
				...
				if ( fabs( accessMat( surface, i, j+1 ) - accessMat( surfaceCopy, i, j+1 ) > global_residual ) ) {
						global_residual = fabs( accessMat( surface, i, j+1 ) - accessMat( surfaceCopy, i, j+1 ) );
		...
		/* 4.3. Move teams */
		#pragma omp parallel for schedule(static,4),private(i,j)
		for( t=0; t<num_teams; t++ ) {
		...
		/* 4.4. Team actions */
		#pragma omp parallel for schedule(static,2),private(t,i,j)
		for( t=0; t<num_teams; t++ ) {
