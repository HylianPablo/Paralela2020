 ...
 */
	...
	#pragma omp parallel for default(none) private(i, j) firstprivate(rows, columns) shared(surface)
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j+=4 ){
			accessMat( surface, i, j ) = 0.0;
			accessMat( surface, i, j+1 ) = 0.0;
			accessMat( surface, i, j+2 ) = 0.0;
			accessMat( surface, i, j+3 ) = 0.0;
		}
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		//#pragma omp parallel default(none) shared(surface, surfaceCopy, focal, num_deactivated) firstprivate(nun_focal, iter, rows, columns) private(i, j)
		//{
			/* 4.1. Activate focal points */
			#pragma omp parallel for default(none) shared(focal) firstprivate(num_focal, iter) private(i) reduction(+:num_deactivated)
			for( i=0; i<num_focal; i++ ) {
				...
				if ( focal[i].active == 2 ) {
					num_deactivated = num_deactivated + 1;
				}
			...
			for( step=0; step<10; step++ )	{
				#pragma omp parallel default(none) private(i, j) firstprivate(num_focal, columns, rows) shared(focal, surface, surfaceCopy, global_residual)
				{
			...
			/* 4.3. Move teams */
			#pragma omp parallel for default(none) private(t, j) firstprivate(num_teams, num_focal) shared(focal, teams)
			for( t=0; t<num_teams; t++ ) {
				/* 4.3.1. Choose nearest focal point */
				float distance = FLT_MAX;
				int target = -1;
				//#pragma omp parallel default(none) private(j) firstprivate(t, num_teams, num_focal) shared(focal, teams, distance, target)
				//{
					//#pragma omp for
			...
			/* 4.4. Team actions */
			#pragma omp parallel for default(none) private(t, i, j) firstprivate(num_teams, rows, columns) shared(focal, teams, surface) 
			for( t=0; t<num_teams; t++ ) {
				...
				//#pragma omp parallel for default(none) private(i, j) firstprivate(rows, columns, radius, t) shared(teams, surface)
