 ...
 */
 ...
 #pragma omp parallel for default(none)private(i,j)shared(surface)firstprivate(rows,columns)
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ )
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			float* temporal=surfaceCopy;
			surfaceCopy=(float*) surface;
			#pragma omp parallel for default (none) shared(surfaceCopy,num_deactivated,num_focal,temporal)private(i,j) firstprivate(rows,columns)reduction(max:global_residual)
			/* 4.2.3. Update surface values (skip borders) */
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ){
					accessMat( temporal, i, j ) = ( 
						accessMat( surfaceCopy, i-1, j ) +
						accessMat( surfaceCopy, i+1, j ) +
						accessMat( surfaceCopy, i, j-1 ) +
						accessMat( surfaceCopy, i, j+1 ) ) / 4;
						if(num_focal==num_deactivated&&(accessMat(temporal,i,j)-accessMat(surfaceCopy,i,j)>global_residual)){
							global_residual=accessMat(temporal,i,j)-accessMat(surfaceCopy,i,j);
						}	
				}
				surface=(float*)temporal;
	...
	if( num_deactivated == num_focal && global_residual < THRESHOLD ) flag_stability = 1;
		...
		/* 4.3. Move teams */
		//#pragma omp parallel  shared(rows,columns,surface,num_teams,num_focal,focal,teams)
		{
		...
		#pragma omp parallel for  default(none)private(t,i,j)shared(focal,teams,surface)firstprivate(num_teams,rows,columns)
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					// cambio para evitar usar sqrtf
					//dx*dx+dy*dy =radius al cuadrado
