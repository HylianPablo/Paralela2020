// User: g108@79.109.105.58 
// ExecutionRequest[P:'2pa.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 00:37:23
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			//#pragma omp parallel for default(none) shared(focal,surface) 
	...
	surfaceCopy=aux;
			...
			/* 4.2.3. Update surface values (skip borders) */
									//REGION 3
	#pragma omp parallel for default(none) shared(surface,surfaceCopy) firstprivate(rows,columns) private(i,j)
			for( i=1; i<rows-1; i++ ) {  
				for( j=1; j<columns-1; j++ ){
			...
			/* 4.2.4. Compute the maximum residual difference (absolute value) */
													//REGION 4
	float aux2=0;												
	#pragma omp parallel for default(none) shared(surface,surfaceCopy) firstprivate(rows,columns) private(i,j,aux2) reduction(max:global_residual)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) {
					aux2=fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ));
					if( aux2 >global_residual) {
						global_residual =aux2;
					}
				}
		}
		...
		/* 4.3. Move teams */
		#pragma omp parallel for default(none) private(j) firstprivate(num_focal,num_teams,focal) shared(teams) 
		for( t=0; t<num_teams; t++ ) {
		...
		/* 4.4. Team actions */
		#pragma omp parallel for default(none) firstprivate(teams,num_teams,rows,columns,focal) private (i,j) shared(surface)	
			for( t=0; t<num_teams; t++ ) {
	...
	//sprintf("Region1: %f, Region2: %f, Region3: %f, Region4: %f, Region5: %f",region1,region2,region3,region4,region5);
/*
 *
