// User: g111@88.13.194.146 
// ExecutionRequest[P:'smurf.c',P:1,T:1,args:'',q:'openmplb'] 
// Mar 15 2019 19:43:35
 ...
 */
	...
	#pragma omp parallel for shared(surface), private(i,j) //collapse(2)//schedule(static, 64)
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ )
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		//#pragma parallel for reduction(+:num_deactivated)
		for( i=0; i<num_focal; i++ ) {
		...
		float global_residual1 = 0.0f;
		int step;
		float * temp;
		for( step=0; step<10; step++ )	{
			/* 4.2.1. Update heat on active focal points */
			//#pragma omp parallel shared(surface,surfaceCopy,focal) 
			//{
				for( i=0; i<num_focal; i++ ) {
				...
				#pragma omp parallel 
				{
				#pragma omp for private(i,j) reduction(max:global_residual) 
				for( i=1; i<rows-1; i++ ){
					for( j=1; j+1 <columns-1; j+=2 ){
						...
						accessMat( surface, i, j+1 ) = ( 
							...
							accessMat( surfaceCopy, i, j+2 ) ) / 4;

				if(step==0){	
						if ( /*fabs*/( accessMat( surface, i, j+1 ) - accessMat( surfaceCopy, i, j+1 ) > global_residual ) ) {
							global_residual = ( accessMat( surface, i, j+1 ) - accessMat( surfaceCopy, i, j+1 ) );
						}
					}
				}
				...
				for( i=1; i<rows-1; i++ )
					for( j=1; j<columns-1; j++ ) 
						// Bug al realizar la comprobacion --> Realiza el absoluto de la comprobacion
						// cuando tendría que realizar el absoluto de la resta y luego comparar
						if ( fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) > global_residual ) ) {
							#pragma omp atomic write // Ejecutan la operacion de escritura en exclusion mutua
		...
		/* 4.3. Move teams */
		#pragma omp parallel for shared(focal,teams) private(t,j) //schedule(dynamic, 80)
		for( t=0; t<num_teams; t++ ) {
			/* 4.3.1. Choose nearest focal point */
			float distance = FLT_MAX;
			int target = -1;
			//#pragma omp parallel for shared(target) private(j)
		...
		/* 4.4. Team actions */
		#pragma omp parallel for default(none) shared(focal,teams, num_teams,surface) firstprivate(columns,rows) private(t, i, j) //reduction(*:surface) //schedule(dynamic, 80) //<-- Da error en el resultado
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if (distance <= radius*radius) {
						#pragma omp atomic
						accessMat( surface, i, j ) *=(0.75); //accessMat( surface, i, j ) * ( 1 - 0.25 ); // Team efficiency factor
					}
				}
			}
		}
