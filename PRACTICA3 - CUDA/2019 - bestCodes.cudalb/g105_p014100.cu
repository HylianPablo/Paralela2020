 ...
 *
 */
__global__ void propagate_kernel(float *TO, float *FROM, int rows, int columns, int paddingColumns) {
	...
	accessMat( TO, i, j ) = ( 
									accessMat( FROM, i-1, j ) +
									accessMat( FROM, i+1, j ) +
									accessMat( FROM, i, j-1 ) +
									accessMat( FROM, i, j+1 ) ) / 4;
}
 ...
 * Move teams 
 */
 __global__ void team_movement_kernel(Team *teams, int num_teams, FocalPoint *focal, int num_focal) {
	...
	for( j=0; j<num_focal; j++ ) {
		...
		float squared_local_distance = (focal[j].x - teams[gid].x)*(focal[j].x - teams[gid].x) + (focal[j].y - teams[gid].y)*(focal[j].y - teams[gid].y);
	...
	if ( teams[gid].type == 1 ) { 
		...
		if ( focal[target].x < teams[gid].x ) teams[gid].x--;
		else if ( focal[target].x > teams[gid].x ) teams[gid].x++;
		if ( focal[target].y < teams[gid].y ) teams[gid].y--;
		else if ( focal[target].y > teams[gid].y ) teams[gid].y++;
	}
	else if ( teams[gid].type == 2 ) { 
 ...
 */
__global__ void update_heat_kernel(float *surface, int rows, int paddingColumns, FocalPoint *focal, int num_focal) {

	int gid = blockIdx.x * blockDim.x + threadIdx.x;

	if (gid >= num_focal) return;

	if (focal[gid].active != 1) return;
	
	//printf("Hilo %d - Foco x=%d , y=%d, heat=%d\n", gid, focal[gid].x, focal[gid].y, focal[gid].heat);
 ...
 * Init surface 
 */
__global__ void init_kernel(float *surface, int rows, int paddingColumns) {
	int gid = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
 * Reduce heat arount the team.
 */
 __global__ void team_action(float *surface, Team * teams, int rows, int columns, int paddingColumns, int t, int squared_radius) {
 ...
 */
	...
	int blockSize = NUM_THREADS_PER_BLOCK;
	int gridSize = numBlocks;


	/* Allocate surface and surfaceCopy in DEVICE */
	float *devSurface, *devSurfaceCopy;
	...
	/* Allocate array to store the global_residual in DEVICE */
	float *dev_global_residual;
	cudaMalloc((void **) &dev_global_residual, numValues*sizeof(float));
	CHECK_ERROR("allocating dev_global_residual");

	
	/* Allocate focal points in DEVICE */
	FocalPoint *devFocal;
	cudaMalloc((void **) &devFocal, sizeof(FocalPoint) * num_focal);
	CHECK_ERROR("allocating devFocal");

	/* Allocate teams in DEVICE */
	Team *devTeams;
	cudaMalloc((void **) &devTeams, num_teams * sizeof(Team));
	CHECK_ERROR("allocating devTeams");


	/* Move teams from HOST to DEVICE */
	cudaMemcpy(devTeams, teams, num_teams*sizeof(Team), cudaMemcpyHostToDevice);
	CHECK_ERROR("copying teams from HOST to DEVICE");


	/* 3. Initialize surface */
	init_kernel<<< gridSize, blockSize>>>(devSurface, rows, paddingColumns);
	CHECK_ERROR("Initializing surface");
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		/* Copy focal points from HOST to DEVICE */
		cudaMemcpy(devFocal, focal, num_focal * sizeof(FocalPoint), cudaMemcpyHostToDevice);
		CHECK_ERROR("copying focal points from HOST to DEVICE");		
		...
		for( step=0; step<10; step++ )	{
			...
			if (step % 2 == 0) {
				...
				update_heat_kernel<<<gridSize, blockSize>>>(devSurface, rows, paddingColumns, devFocal, num_focal);
				CHECK_ERROR("update_heat_kernel 1");

				/* 4.2.3. Update surface values  TO devSurfaceCopy  FROM  devSurface */
				propagate_kernel<<<gridSize, blockSize>>>(devSurfaceCopy, devSurface, rows, columns, paddingColumns);
				CHECK_ERROR("propagate_kernel 1");

			} else {

				/* 4.2.1. Update heat on active focal points */
				update_heat_kernel<<< gridSize, blockSize >>>(devSurfaceCopy, rows, paddingColumns, devFocal, num_focal);
				CHECK_ERROR("update_heat_kernel 2");

				/* 4.2.3. Update surface values TO devSurface FROM devSurfaceCopy */
				propagate_kernel<<<gridSize, blockSize>>>(devSurface, devSurfaceCopy, rows, columns, paddingColumns);
				CHECK_ERROR("propagate_kernel 2 ");
			}
			...
			if (num_deactivated == num_focal && !thresshold_passed) {
				...
				difference_kernel<<<gridSize, blockSize>>>(dev_global_residual, devSurfaceCopy, devSurface, rows, columns, paddingColumns);
				CHECK_ERROR("difference_kernel");
	

				/* Reduction of global_residual */
		...
		if (num_focal != num_deactivated) {
			...
			/* 4.3. Move teams */
			team_movement_kernel<<< gridSize, blockSize>>>(devTeams, num_teams, devFocal, num_focal);
			CHECK_ERROR("team movement");

			cudaMemcpy(teams, devTeams, num_teams*sizeof(Team), cudaMemcpyDeviceToHost);
			CHECK_ERROR("copying teams from HOST to DEVICE");
			...
			if ( teams[t].type == 1 ) {
				...
				for( i=teams[t].x-RADIUS_TYPE_1; i<=teams[t].x+RADIUS_TYPE_1; i++ ) {
					for( j=teams[t].y-RADIUS_TYPE_1; j<=teams[t].y+RADIUS_TYPE_1; j++ ) {
						...
						if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j) <= SQUARED_RADIUS_TYPE_1 ) {
							accessMat( surface, i, j ) = accessMat( surface, i, j ) * 0.75; // Team efficiency factor
						}
					}
				}
				...
				for( i=teams[t].x-RADIUS_TYPE_2_3; i<=teams[t].x+RADIUS_TYPE_2_3; i++ ) {
					for( j=teams[t].y-RADIUS_TYPE_2_3; j<=teams[t].y+RADIUS_TYPE_2_3; j++ ) {
						...
						if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j) <= SQUARED_RADIUS_TYPE_2_3 ) {
							accessMat( surface, i, j ) = accessMat( surface, i, j ) * 0.75; // Team efficiency factor
						}
					}
				}
	...
	cudaMemcpy(devSurface, surface, realNumValues*sizeof(float), cudaMemcpyHostToDevice);
