// User: g111@88.10.101.212 
// ExecutionRequest[P:'cucucuda.cu',P:1,T:1,args:'',q:'cudalb'] 
// May 16 2019 17:13:01
...
Funcion del device  
**********************************************/
__global__ void updateHeat(FocalPoint *dFocal, float *dSurface,int num_focal,int columns){
	...
	if ( position < num_focal ){
		if ( dFocal[position].active == 1 ){
			int x = dFocal[position].x;
			int y = dFocal[position].y;
			accessMat( dSurface, x, y ) = dFocal[position].heat;
		}
	}
}

__global__ void copyValues(float *dSurface,float *dSurfaceCopy,int rows,int columns){
	/* Posición global del hilo */	
	int position = (threadIdx.x + blockIdx.x * blockDim.x) + (threadIdx.y + blockIdx.y * blockDim.y) * blockDim.x * gridDim.x;
...
__global__ void updateValues(float *dSurface,float *dSurfaceCopy,int rows,int columns){
	/* Posición global del hilo */	
	int position = (threadIdx.x + blockIdx.x * blockDim.x) + (threadIdx.y + blockIdx.y * blockDim.y) * blockDim.x * gridDim.x;
	int first = columns - 1;
...
__global__ void reduce(float *dSurface, float *dSurfaceCopy, int rows , int columns,float *dMaximos){
...
__global__ void reduceHeat(float *dSurface,FocalPoint *dFocal, Team *dTeams,int rows,int columns,int num_teams){
	
	int position = (threadIdx.x + blockIdx.x * blockDim.x) + (threadIdx.y + blockIdx.y * blockDim.y) * blockDim.x * gridDim.x;
	if(position < num_teams) {
		/* 4.4.1. Deactivate the target focal point when it is reached */
		int target = dTeams[position].target;
		if ( target != -1 && dFocal[target].x == dTeams[position].x && dFocal[target].y == dTeams[position].y 
			&& dFocal[target].active == 1 )
			dFocal[target].active = 2; 

		/* 4.4.2. Reduce heat in a circle around the team */
		int radius;
		// Influence area of fixed radius depending on type
		if ( dTeams[position].type == 1 ) radius = RADIUS_TYPE_1;
		else radius = RADIUS_TYPE_2_3;
		for(int i = dTeams[position].x-radius; i <= dTeams[position].x+radius; i++ ) {
			for(int j = dTeams[position].y-radius; j <= dTeams[position].y+radius; j++ ) {
				if ( i<1 || i>=rows-1 || j<1 || j>=columns-1 ) continue; // Out of the heated surface
				float dx = dTeams[position].x - i;
				float dy = dTeams[position].y - j;
				float distance = sqrtf( dx*dx + dy*dy );
				if ( distance <= radius ) {
					atomicMul(&accessMat( dSurface, i, j ),0.75);
				}
			}
		}
	}
...
/*global void partefinal(float *surfaceGPU,float *surfaceCopyGPU,int rows,int columns,Team *teamsGPU, FocalPoint *focalGPU, int num_teams){
 ...
 */
	...
	/* Reserva de memoria para las variables del dispositivo (memoria glbal GPU) */
	float *dSurface;
	float *dSurfaceCopy;
	Team *dTeams;
	FocalPoint *dFocal;
	
	error =  cudaMalloc((void **) &dSurface, sizeof(float) * (size_t)rows * (size_t)columns);
	if(error != cudaSuccess){
		printf("Error: %s\n",cudaGetErrorString(error));
	}
	error = cudaMalloc((void **) &dSurfaceCopy, sizeof(float) * (size_t)rows * (size_t)columns);
	if(error != cudaSuccess){
		printf("Error: %s\n",cudaGetErrorString(error));
	}
	error = cudaMalloc((void **) &dTeams, sizeof(Team) * (size_t)num_teams);
	if(error != cudaSuccess){
		printf("Error: %s\n",cudaGetErrorString(error));
	}
	error = cudaMalloc((void **) &dFocal, sizeof(FocalPoint) * (size_t)num_focal);
	if(error != cudaSuccess){
		printf("Error: %s\n",cudaGetErrorString(error));
	}

	/* Declaración y obtención del shape de los bloques y del grid */
	/* X = 32 Y = 8*/
	int tamBlockX = 32;
	int tamBlockY = 8;
	//int tamBlock = tamBlockX * tamBlockY;
	...
	dim3 blockShape(tamBlockX,tamBlockY);
	dim3 gridShape(tamGridX,tamGridY);

	/* Variables usadas para reduccion --> Global Residual */
	float *maximos = (float *)malloc( sizeof(float) * tamGridX * tamGridY );
	float *dMaximos;

	error = cudaMalloc((void **) &dMaximos, sizeof(float) * tamGridX * tamGridY );
	if(error != cudaSuccess){
		printf("Error: %s\n",cudaGetErrorString(error));
	}
	...
	*/
	...
	int first_activation = focal[0].start;
	for(i = 1; i < num_focal;i++){
		if(focal[i].start < first_activation){
			first_activation = focal[i].start;
		}
	}
	
	for( iter=first_activation; iter<max_iter && ! flag_stability; iter++ ) {
		...
		error = cudaMemcpy(dFocal,focal, sizeof(FocalPoint)* num_focal,cudaMemcpyHostToDevice);
		if(error != cudaSuccess){
			printf("Error: %s\n",cudaGetErrorString(error));
		}
		...
		for( step=0; step<10; step++ )	{
			...
			updateHeat<<<gridShape,blockShape>>>(dFocal,dSurface,num_focal,columns);
			...
			copyValues<<<gridShape,blockShape>>>(dSurface,dSurfaceCopy,rows,columns);
			error = cudaGetLastError();
			if(error != cudaSuccess){
				printf("Error: %s\n",cudaGetErrorString(error));
			}
			...
			updateValues<<<gridShape,blockShape>>>(dSurface,dSurfaceCopy,rows,columns);
			error = cudaGetLastError();
			if(error != cudaSuccess){
				printf("Error: %s\n",cudaGetErrorString(error));
			}
			...
			if(step == 0 && num_deactivated == num_focal){
				...
				reduce<<<gridShape,blockShape,tamSize>>>(dSurface,dSurfaceCopy,rows,columns,dMaximos);
				error = cudaGetLastError();
				if(error != cudaSuccess){
					printf("Error: %s\n",cudaGetErrorString(error));
				}
					
				error = cudaMemcpy(maximos,dMaximos, sizeof(float) * tamGridX * tamGridY,cudaMemcpyDeviceToHost);
				if(error != cudaSuccess){
					printf("Error: %s\n",cudaGetErrorString(error));
				}

				for( int count = 0; count < (tamGridX * tamGridY);count++){
					if(maximos[count] > global_residual){
						global_residual = maximos[count];
					}
				}
			}
		}

		/*error = cudaMemcpy(surface,dSurface, sizeof(float)*(rows*columns),cudaMemcpyDeviceToHost);
		if(error != cudaSuccess){
			printf("Error: %s\n",cudaGetErrorString(error));
		}

		error = cudaMemcpy(focal,dFocal, sizeof(FocalPoint)*num_focal,cudaMemcpyDeviceToHost);
		if(error != cudaSuccess){
			printf("Error: %s\n",cudaGetErrorString(error));
		}*/
		...
		error = cudaMemcpy(dTeams,teams, sizeof(Team)* num_teams,cudaMemcpyHostToDevice);
		if(error != cudaSuccess){
			printf("Error: %s\n",cudaGetErrorString(error));
		}
		...
		reduceHeat<<<gridShape,blockShape>>>(dSurface,dFocal,dTeams,rows,columns,num_teams);
		error = cudaGetLastError();
		if(error != cudaSuccess){
			printf("Error: %s\n",cudaGetErrorString(error));
		}
