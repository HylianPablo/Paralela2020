...
// ExecutionRequest[P:'atomarpor.cu',P:1,T:1,args:'',q:'cudalb'] 
...
__global__ void updateValues(float *surface, float *surfaceCopy, int rows, int columns){
	...
	if(x>0 && x<rows-1 && y>0 && y<columns-1){
		accessMat( surface, x, y ) = ( 
			accessMat( surfaceCopy, x-1, y ) +
			accessMat( surfaceCopy, x+1, y ) +
			accessMat( surfaceCopy, x, y-1 ) +
			accessMat( surfaceCopy, x, y+1 ) ) / 4;
	}
	
}

__global__ void reduce0(float *deviceSurfaceCopy, float *deviceSurface, int rows, int columns, float *d_max){
	...
	if(position<rows*columns){
		float valor = accessMat(deviceSurface, i, j) - accessMat(deviceSurfaceCopy, i, j);
		if(valor<0){
			valor=valor*(-1);
		}
		sdata[id] = valor;
	...
	if(id == 0) d_max[blockIdx.x + blockIdx.y * gridDim.x] = sdata[0];
}

__global__ void updateHeat(float *matriz, FocalPoint *pf, int num_focal,int columns){


	for( int i=0; i<num_focal; i++ ) {
		if ( pf[i].active != 1 ) continue;
		int x = pf[i].x;
		int y = pf[i].y;

		accessMat(matriz, x, y) = pf[i].heat;
	}
}
 ...
 */
	...
	dim3 blockdim(tamBlockX,tamBlockY);
	dim3 griddim(tamGridX,tamGridY);

	cudaMalloc((void**)&deviceSurface, (size_t)rows*(size_t)columns*sizeof(float));
	cudaMalloc((void**)&deviceSurfaceCopy, (size_t)rows*(size_t)columns*sizeof(float));
	cudaMalloc((void**)&d_max, sizeof(float)*(size_t)tamGridX*(size_t)tamGridY);
	cudaMalloc((void**)&fp_device, num_focal*sizeof(FocalPoint));
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		cudaMemcpy(deviceSurface, surface, (size_t)rows*(size_t)columns*sizeof(float), cudaMemcpyHostToDevice);
		cudaMemcpy(fp_device, focal, num_focal*sizeof(FocalPoint), cudaMemcpyHostToDevice);
		...
		for( step=0; step<10; step++ )	{
			...
			updateHeat<<<1,1>>>(deviceSurface, fp_device, num_focal,columns);
			...
			//cudaMemcpy(deviceSurface, surface, (size_t)rows*(size_t)columns*sizeof(float), cudaMemcpyHostToDevice);
			//cudaMemcpy(deviceSurfaceCopy, surfaceCopy, (size_t)rows*(size_t)columns*sizeof(float), cudaMemcpyHostToDevice);
			float *tmp;
			tmp = deviceSurfaceCopy;
			deviceSurfaceCopy = deviceSurface;
			deviceSurface = tmp;
			...
			updateValues<<<griddim, blockdim>>>(deviceSurface, deviceSurfaceCopy, rows, columns);
			/*for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ )
			...
			//cudaMemcpy(surface, deviceSurface, (size_t)rows*(size_t)columns*sizeof(float), cudaMemcpyDeviceToHost);
			

			/* 4.2.4. Compute the maximum residual difference (absolute value) */
			if(step == 0){
			reduce0<<<griddim, blockdim,blockdim.x*blockdim.y*sizeof(float)>>>(deviceSurfaceCopy, deviceSurface, rows, columns, d_max);
			cudaMemcpy(maximos, d_max, sizeof(float)*(size_t)tamGridX*(size_t)tamGridY, cudaMemcpyDeviceToHost);
