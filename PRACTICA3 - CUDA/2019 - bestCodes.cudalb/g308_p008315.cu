// User: g308@157.88.125.192 
// ExecutionRequest[P:'prueba1.cu',P:1,T:1,args:'',q:'cudalb'] 
// May 11 2019 12:37:52
...
__global__ void kernel1(float *surface, int columns){	//Usar el size para saber el tamaño de la matriz y alinear los bloques de memoria
/* Identificadores y tamaños necesarios */
        int IDX_Thread = threadIdx.x;				// Identificación del hilo en la dimensión x
        int IDY_Thread = threadIdx.y;				// Identificación del hilo en la dimensión x

        int IDX_block = blockIdx.x;					// Identificación del bloque en la dimensión x
        int IDY_block = blockIdx.y;					// Identificación del bloque en la dimensión x
        
        int threads_per_block = blockDim.x;			// Numero de hilos por bloque

        /* Fórmula para calcular la posición */
        int pos_x = IDX_block * threads_per_block + IDX_Thread; // Elemento del vector como función de hilo y bloque
        int pos_y = IDY_block * threads_per_block + IDY_Thread; // Elemento del vector como función de hilo y bloque

        /* Cálculo de la suma correspondiente */
        surface[ (pos_x) * columns + (pos_y) ];
}
 ...
 */
	...
	float *deviceSurface;
	error = cudaMalloc(&deviceSurface, sizeof(float) * (size_t)rows * (size_t)columns);
	...
	kernel1<<<blocks_per_grid, threads_per_block>>>(deviceSurface, columns);			//************************************************** QUITAR EL 0 POR EL TAMAÑO DE LA MATRIZ CUANDO QUIERA ACCESOS COALESCENTES

	error = cudaMemcpy(surface ,deviceSurface, sizeof(float) * (size_t)rows * (size_t) columns, cudaMemcpyDeviceToHost);
	if(error != cudaSuccess){
		printf(cudaGetErrorString(error));
	}
