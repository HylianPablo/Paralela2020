...
__device__ Statistics stats;

__global__ void evolutionGPU(food_t *food, int num_food, food_t *food_spot, int num_food_spot)
{
	return;
}
 ...
 */
#define THREADS 1024
#define BLOCK max(rows*columns, num_cells, max_new_sources)/THREADS + 1
	...
	int max_new_sources = max2(num_new_sources, num_new_sources_spot);
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food_int && num_cells_alive > 0; iter++ ) {
		...
		cudaCheckKernel((evolutionGPU<<<BLOCK, THREADS>>>(food_to_place_d, num_new_sources, food_to_place_spot_d, num_new_sources_spot)));
		...
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				...
				int food = accessMat( culture, cells[i].pos_row / PRECISION, cells[i].pos_col / PRECISION );
