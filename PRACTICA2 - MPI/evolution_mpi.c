...
#define accessMatP(arr, exp1, exp2) arr[(int)(exp1) * columns + (int)(exp2)]
...
void print_status(int iteration, int rows, int columns, float *culture, int num_cells, Cell *cells, int num_cells_alive, Statistics sim_stat)
{
	...
	for (i = 0; i < rows; i++)
	{
		...
		for( j=0; j<columns; j++ ) {
            char symbol;
            if ( accessMatP( culture, i, j ) >= 20 ) symbol = '+';
            else if ( accessMatP( culture, i, j ) >= 10 ) symbol = '*';
            else if ( accessMatP( culture, i, j ) >= 5 ) symbol = '.';
            else symbol = ' ';
...
#define update_time(timer)
	...
	for (iter = 0; iter < max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++)
	{
		...
		for(i=0; i<num_cells;i++){
			if(mine(cells[i].pos_row, cells[i].pos_col)){
				food_to_share_aux[n] = accessMat(culture, cells[i].pos_row, cells[i].pos_col);
		...
		for(i=0;i<num_cells;i++){
			if(rank==0)
			printf("Valor de food_to_share: %f, posiciones: %f (row) , %f (col) \n",food_to_share[i],cells[i].pos_row,cells[i].pos_col);
		}
		...
		for (i = 0; i < free_position; i++) //antes se ejecuta hasta num_cells
		{
			//if (cells[i].alive)
			//{
		...
		// WARNING: num_cells_alive must be correct FOR EVERY PROCESS.
