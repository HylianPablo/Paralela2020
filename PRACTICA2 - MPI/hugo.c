 ...
 */
	...
	*/
	...
	for( i=0; i<rows; i++ ) //<-------------------------------------------------------------------------------------------------------------- Not sure si mejora
		for( j=0; j<columns; j++ ){
			accessMat( culture, i, j ) = 0.0;
			accessMat( culture_cells, i, j ) = 0; //<------------------------------------------------------------------------------------Podria ser mejor meterlo aqui
		}
...
*/
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		for (i=0; i<num_new_sources; i++) {
			...
			food = (float)( food_level * erand48( food_random_seq ));
		...
		if ( food_spot_active ) {
			...
			for (i=0; i<num_new_sources; i++) {
				row = food_spot_row + (int)(food_spot_size_rows * erand48( food_spot_random_seq ));
				col = food_spot_col + (int)(food_spot_size_cols * erand48( food_spot_random_seq ));
				food = (float)( food_spot_level * erand48( food_spot_random_seq ));
				accessMat( culture, row, col ) = accessMat( culture, row, col ) + food;
			}
		}
		...
		/* <------------------------------------------------------------------------------------------------------------Deberiamos poder hacerlo en la seccion 3.1
		for( i=0; i<rows; i++ )
			for( j=0; j<columns; j++ ) 
		...
		*/
		...
		for( i=0; i<num_cells; i++ ) {
			if ( cells[i].alive ) {
				accessMat( culture, cells[i].pos_row, cells[i].pos_col ) = 0.0f; //<------------------------------------------------------------------------ Prueba
				alive_in_main_list ++;
				if ( free_position != i ) {
		...
		float current_max_food_aux = 0.0f;

		short *culture_cells_aux = (short *)malloc( sizeof(short) * my_size);
		float *culture_aux = (float *)malloc( sizeof(float) * my_size);

		for (i=0; i < my_size; i++){

			culture_cells_aux[i] = 0.0f;
			culture_aux[i] = culture[my_begin+i] * 0.95;

			if(culture_aux[i] > current_max_food_aux){
				current_max_food_aux = culture_aux[i];
			}

		}

		int array_my_size[num_procs];
		int array_desplazamientos[num_procs];
		int offset=0;	

		for(i=0; i<num_procs; i++){

			array_desplazamientos[i] = offset;
			array_my_size[i] = my_size;
			offset += my_size;
		}	

		MPI_Reduce(&current_max_food_aux, &current_max_food, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
		MPI_Gatherv(culture_cells_aux, my_size, MPI_SHORT, culture_cells, array_my_size, array_desplazamientos, MPI_SHORT, 0, MPI_COMM_WORLD);
		MPI_Gatherv(culture_aux, my_size, MPI_FLOAT, culture, array_my_size, array_desplazamientos, MPI_FLOAT, 0, MPI_COMM_WORLD);
		...
		if(my_rank < resto ){
			my_size=my_size+1;
			my_begin = my_size*my_rank ;
		}else{
			my_begin = resto + my_size*my_rank;
		}

		short culture_cells_copy*;
		float culture_copy*;

		memcpy(culture_cells_copy, culture_cells, rows*columns*sizeof(short)); // copia los dos primeros elementos de v2 en v1
		memcpy(culture_copy, culture, rows*columns*sizeof(float)); // copia los dos primeros elementos de v2 en v1

		short aux1[my_size]; //culture_cells
		float aux2[my_size]; //culture


		current_max_food = 0.0f;
		for( i=0; i<my_size; i++ ){
			aux1[i] = 0; //<------------------------------------------------ Para que no de errores al combinar for de la seccion 3.1 y 4.2.1
			aux2[i] = culture[my_begin+i] * 0.95f; // Reduce 5%
			if ( culture[i] > current_max_food ){
				//current_max_food = culture[i];
				MPI_Reduce(&culture[i], &current_max_food, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
			} 
		}
		MPI_Gatherv(aux1, my_size, MPI_FLOAT, culture_cells, receive_counts, receive_displacements, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Gatherv(aux2, my_size, MPI_FLOAT, culture, receive_counts, receive_displacements, MPI_INT, 0, MPI_COMM_WORLD);*/
