// User: g103@79.109.17.38 
// ExecutionRequest[P:'extinguishing_des.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 20:01:02
...
*/
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			for( i=0; i<num_focal; i++ ) {
				...
				if(x >= desp && x < despSig){
					accessMat( surface, x - desp , y) = focal[i].heat;
				...
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j) <= radius * radius ) {
						...
						if(i >= desp && i < despSig){
								...
								accessMat( surface, i - desp, j ) = accessMat( surface, i - desp, j ) * ( 0.75 ); // Team efficiency factor
	...
	for (i=0; i<num_focal; i++){
		...
		if(aux_x >= desp && aux_x < despSig){
			residualHeatAux[i] = accessMat( surface, aux_x - desp, aux_y);					
		}
...
*/
		...
		for( step=0; step<10; step++ )	{
			...
			for( i=0; i<num_focal; i++ ) {
				...
				/*Pasamos las x e y de posición global a local en función del rank y su desplazamiento*/
					...
					if(x >= desp){
						accessMat( surface, x + 1 - desp , y) = focal[i].heat;
					}
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j) <= radius * radius ) {
						...
						if(i >= desp){
							accessMat( surface, i - desp + 1, j ) = accessMat( surface, i - desp + 1, j ) * ( 0.75 ); // Team efficiency factor
						}
					}
				}
			}
	...
	for (i=0; i<num_focal; i++){
		...
		if(aux_x >= desp){
			residualHeatAux[i] = accessMat( surface, aux_x - desp + 1, aux_y);
				...
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j) <= radius * radius ) {
						...
						if(i >= desp && i < despSig){
								...
								accessMat( surface, i + 1 - desp, j ) = accessMat( surface, i + 1 - desp, j ) * ( 0.75 ); // Team efficiency factor
