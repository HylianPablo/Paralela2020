// User: g214@84.127.184.29 
// ExecutionRequest[P:'thats_a_no_no.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 17:28:00
...
//if ( rank == 0 ) {		Secuencial
	MPI_Comm_size(MPI_COMM_WORLD, &num_procs);

	//DIVISION DE TRABAJO POR FILAS

	//reparto de X filas entre Y procesos: cada proceso tiene X/Y, y se reparte el resto de la division entera entre los primeros procesos.
	
	int surface_slice_tam =  (rank < rows%num_procs) ? (rows/num_procs) + 3 : (rows/num_procs)+2;
	for(i = 0; i < num_focal; i++){
		residualHeat[i] = 0.0f;
	}
	
	// rows - (surface_slice_tam*(num_procs-rank))
	int slice_b, slice_e;
	if(rank<rows%num_procs){
	slice_b = rank*(surface_slice_tam-2);
	slice_e = (rank+1)*(surface_slice_tam-2);
	}else{
	slice_b = rows-((surface_slice_tam-2)*((num_procs-rank)));
	slice_e = rows-((surface_slice_tam-2)*((num_procs-rank)-1));
	}
	//int focal_slice_tam = (rank < num_focal%num_procs) ? (num_focal/num_procs) + 1: (num_focal/num_procs); //Cacho del array de focos
	//int teams_slice_tam = (rank < num_teams%num_procs) ? (num_te)ams/num_procs) + 1: (num_teams/num_procs); //Cacho del array de teams


	float *surface = (float *)malloc( sizeof(float) * (size_t)(surface_slice_tam) * (size_t)columns );
	float *surfaceCopy = (float *)malloc( sizeof(float) * (size_t)(surface_slice_tam) * (size_t)columns );
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			for( i=0; i<num_focal; i++ ) {
				...
				int x = focal[i].x;
				if ( (x < slice_e && x >= slice_b)){				// Si no pertenece al conjunto de filas
					int y = focal[i].y;
					accessMat( surface, x-slice_b+1, y ) = focal[i].heat;	// Si pertenece a la seccion asignar calor	( +1 por halo ) 
				}
			}
			...
			*/
			...
			MPI_Request a1 , a2;
			if(rank>0) MPI_Isend(surface+(columns), columns, MPI_FLOAT, rank-1, 0 , MPI_COMM_WORLD, &a1);	// Send 1
			MPI_Status stat1, stat2;
			if(rank<num_procs-1) MPI_Isend(surface+(columns*(surface_slice_tam-2)), columns,MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD, &a2); // Send 2
			
			if(rank<num_procs-1) MPI_Recv(surfaceCopy+(columns*(surface_slice_tam-1)), columns, MPI_FLOAT, rank+1, 0 ,MPI_COMM_WORLD, &stat1 ); 	// Recv 1
			if(rank>0) MPI_Recv(surfaceCopy, columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD, &stat2);	//Recv 2

			

			/* 4.2.3. Update surface values (skip borders) */
			
			for( i=1; i<surface_slice_tam-1; i++ ){
				if(rank==0 && i==1) continue;
				if(rank==num_procs-1 && i==surface_slice_tam-2) continue;
				...
				if (step==0 && (num_focal==num_deactivated) &&fabs( accessMat( temp, i, j ) - accessMat( surfaceCopy, i, j ) > global_residual )  ) {
						global_residual = fabs( accessMat( temp, i, j ) - accessMat( surfaceCopy, i, j ) );
					}
			...
			//		}
			// Como global_residual es un maximo, hacemos que se unan entre todos los procesos.
			
		}
		float tot = global_residual;
		MPI_Allreduce(&tot,&global_residual, 1 , MPI_FLOAT, MPI_MAX , MPI_COMM_WORLD );	// ALL quiere decir que lo ejecutan todos los procesos
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					if (i<1 || i>=rows-1 || j<1 || j>=columns-1 || i >= slice_e || i < slice_b) continue; // Out of the heated surface and slice
						//if(i<slice_e && i >= slice_b){					
							float dx = teams[t].x - i;
							float dy = teams[t].y - j;
							float distance =  dx*dx + dy*dy;
							
							// ignore those who dont correspond
							int ii = (i-slice_b)+1;
							
							if ( distance <= radius*radius && accessMat(surface, ii, j) > 0) {
								accessMat( surface, ii, j ) = accessMat( surface, ii, j ) * ( 0.75 ); // Team efficiency factor
							}
						//}
				}
			}
		...
		print_status( iter, surface_slice_tam, columns, surfaceCopy, num_teams, teams, num_focal, focal, global_residual );
	...
	// MAKE ORDERED AAAAAAAAAAAAAAAA
	///////////////////
	/////////////////7
	/////////////////////
	////////////////////
	
	float *localHeat = (float*)malloc( sizeof(float) * (size_t)num_focal );
	
	for (i=0; i<num_focal; i++){
		localHeat[i]=0.0f;	
		if( (focal[i].x<slice_e && focal[i].x >= slice_b) ){
			localHeat[i] = accessMat( surface, focal[i].x-(slice_b)+1 , focal[i].y);
		}	
	}
	MPI_Reduce(localHeat, residualHeat, num_focal, MPI_FLOAT, MPI_MAX,0, MPI_COMM_WORLD);
...
for(i=0; i<num_focal; i++){
	localHeat = 0.0f;
	if( (focal[i].x<slice_e && focal[i].x >= slice_b) ){
		localHeat = accessMat( surface, focal[i].x-(slice_b)+1 , focal[i].y);
	}
	MPI_Reduce( &localHeat, &residualHeat[i], 1, MPI_FLOAT, MPI_MAX, 0,MPI_COMM_WORLD );
}
*/
	...
	if(rank==0){
		for (i=0; i<num_focal; i++){
			...
			MPI_Recv(&residualHeat[i], 1, MPI_FLOAT, MPI_ANY_SOURCE, i, MPI_COMM_WORLD, &stat1);
		}	
	}
