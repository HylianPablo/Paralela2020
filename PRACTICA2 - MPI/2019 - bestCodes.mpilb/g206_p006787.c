// User: g206@157.88.139.133 
// ExecutionRequest[P:'tuberialarga.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 17:40:05
 ...
 */
void print_status( int iteration, int rows, int columns, float *surface, int num_teams, Team *teams, int num_focal, FocalPoint *focal, float global_residual,int rank,int rank_max,int start) {
 /* 
 ...
 for( i=1; i<rows-1; i++ ) {
   ...
   */
   float symbol = accessMat(surface,i,j);

   int t;
   int flag_team = 0;
   for( t=0; t<num_teams; t++ ) 
    if ( teams[t].x == i+start-1 && teams[t].y == j ) { flag_team = 1; break; }
   if ( flag_team ) printf("[%f]", symbol );
   else {
    int f;
    int flag_focal = 0;
    for( f=0; f<num_focal; f++ ) 
     if ( focal[f].x == i+start-1 && focal[f].y == j && focal[f].active == 1 ) { flag_focal = 1; break; }
    if ( flag_focal ) printf("(%f)", symbol );
    else printf(" %f ", symbol );
   }
  }
  printf("|\n");
 }
...
// MPI Version: Eliminate this conditional to start doing the work in parallel
	...
	int my_size, start, stop;

	my_size = rows / n_procs;
	start = rank * my_size - 1;

	if(rank==0){
		start = 0;
	}
	if(rows%n_procs != 0 && rank==n_procs-1){
		my_size+=rows%n_procs;
	}	
	
	if(rank==0 || rank==n_procs-1){
		my_size+=1;
	}else{
		my_size+=2; //Añade los dos huecos para los halos
	}
	stop = start + my_size - 1;
	
	// printf("%d %d %d\n", start, stop, my_size);

	float *surface = (float *)malloc( sizeof(float) * (size_t)my_size * (size_t)columns );
	float *surfaceCopy = (float *)malloc( sizeof(float) * (size_t)my_size * (size_t)columns );
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			for( i=0; i<num_focal; i++ ) {
				...
				int y = focal[i].y;
				if(x>=start && x<=stop) 
					accessMat( surface, x-start, y ) = focal[i].heat;
			}
			...
			//Aquí hacer intercambio de halos
			//Intentado primero con Isend sin resultado adecuado. Petó. Vamos con Send
			//Con surface no funcionaba a veces y con surfaceCopy en leaderboard falla y en pc no
			if(rank==n_procs-1){
				MPI_Send(&(accessMat(surfaceCopy, 1, 0)), columns, MPI_FLOAT, rank-1, 1000, MPI_COMM_WORLD);
				MPI_Recv(&(accessMat(surfaceCopy, 0, 0)), columns, MPI_FLOAT, rank-1, 1000, MPI_COMM_WORLD, &stat);
			}else if(rank==0){
				
				MPI_Send(&(accessMat(surfaceCopy, my_size-2, 0)), columns, MPI_FLOAT, rank+1, 1000, MPI_COMM_WORLD);
				
				MPI_Recv(&(accessMat(surfaceCopy, my_size-1, 0)), columns, MPI_FLOAT, rank+1, 1000, MPI_COMM_WORLD, &stat);		
			...
			//Error ahora con el fabs. surface y surfaceCopy siempre están en 0.0 al hacer printf
		...
		//Computando máximo valor de global_residual entre todos los procesos
		MPI_Allreduce(&global_residual, &max_global_residual, 1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD);
		/*if(rank==0){
			printf("%d %f\n", iter, max_global_residual);
		}*/
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if(i>=start && i<=stop){
						...
						if ( distance <= radius ) {
							accessMat( surface, i-start, j ) = accessMat( surface, i-start, j ) * ( 0.75 ); // Team efficiency factor
						}
					}
		...
		for (int i=0;i<n_procs;i++){
   			if (i ==rank){
				  print_status( iter, my_size, columns, surface, num_teams, teams, num_focal, focal, max_global_residual,rank,n_procs-1,start );
			   }
   			MPI_Barrier(MPI_COMM_WORLD);
  		}
	...
	//Al añadir los iguales parece que en el lb petó más tarde, buena señal
	// MPI Version: Store the results of residual heat on focal points
	//Añadidas cositas

	
//Quité el free de aquí. Se estaba realizando dentro del bucle iter
// MPI Version: Eliminate this conditional-end to start doing the work in parallel
}

for (i=0; i<num_focal; i++){
	residualHeat[i] = 0.0f;
	if(focal[i].x>start && focal[i].x<stop){
		residualHeat[i] = accessMat( surface, focal[i].x-start, focal[i].y );
		//printf(" %.6f\n", residualHeat[i] );
		//printf("START = %d\tSTOP = %d\n",start, stop);
	}

}		
float *parcialHeat = (float*)malloc( sizeof(float) * (size_t)num_focal );
MPI_Allreduce(residualHeat, parcialHeat, num_focal, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD);
residualHeat = parcialHeat;
