...
// ExecutionRequest[P:'extinguishing_mpiTRUE.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 18:17:22
 ...
 */
	...
	int iter, lessP;
	int nprocs, p_rows, r_rows, i_row, f_row, i_total, f_total, i_region, f_region;
	MPI_Comm_size( MPI_COMM_WORLD, &nprocs );

   //	NÚMERO DE FILAS DE LA REGIÓN, INICIO Y FIN DE LA REGIÓN GLOBAL

   	p_rows = rows / nprocs;
   	r_rows = rows % nprocs;
   	if(r_rows > 0 && r_rows - rank > 0){
   		p_rows++;
   		i_row = p_rows*rank;
   	} else{
   		i_row =(rank*p_rows)+r_rows;
   	}
   	f_row = i_row+p_rows;
   ...
   //CREACIÓN DE REGIONES
   	...
   	float *surface = (float *)malloc( sizeof(float) * ((size_t)p_rows+1) * (size_t)columns );
   	float *surfaceCopy = (float *)malloc( sizeof(float) * ((size_t)p_rows+1) * (size_t)columns );
   	...
   	if( rank != 0 && rank != nprocs-1 ){
   		surface = (float *)realloc( surface, sizeof(float) * ((size_t)p_rows+2) * (size_t)columns );
   		surfaceCopy = (float *)realloc( surfaceCopy, sizeof(float) * ((size_t)p_rows+2) * (size_t)columns );
   	}


   //INICIALIZACIÓN A 0 DE SURFACE Y BORDES DE SURFACECOPY

   	for ( i=i_total; i < f_total; i++){
   	  for( j=0; j<columns; j++ ){
   ...
   //	PASOS
          ...
          // SEND DE HALOS
   		    ...
   		    for( i=0; i<num_focal; i++ ) {
   		      ...
   		      int y = focal[i].y;
   		      if(x >= i_row - i_region){
   		        if(x < i_row - i_region + f_total ){
   		          accessMat( surfaceCopy, x-i_row+i_region, y ) = focal[i].heat;
   						}
   					}
   		    }
   		    /* 4.2.3. Update surface values (skip borders) */
   		    for( i=1; i<f_total-1; i++ ){
   		    ...
   		    /* 4.2.4. Compute the maximum residual difference (absolute value) */
   		        ...
   		        for( i=i_region; i<f_region; i++ ){
   		          if( global_residual >= THRESHOLD ) break;
   		          for( j=1; j<columns-1; j++ ){
   		            if ( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j )  >= THRESHOLD ) {
          ...
          // SEND DE HALOS
       		if( rank==0 ){
       			MPI_Send( &accessMat( surface, p_rows-1, 0 ), columns, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD );
       		} else if( rank==nprocs-1 ) {
       			MPI_Send( &accessMat( surface, 1, 0 ), columns, MPI_FLOAT, rank-1, 2, MPI_COMM_WORLD );
       		} else {
       			MPI_Send( &accessMat( surface, p_rows, 0 ), columns, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD );
       			MPI_Send( &accessMat( surface, 1, 0 ), columns, MPI_FLOAT, rank-1, 2, MPI_COMM_WORLD );
       		}
       		// RECV DE HALOS
       		if( rank==0 ){
       			MPI_Recv( &accessMat( surface, p_rows, 0 ), columns, MPI_FLOAT, rank+1, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
       		} else if( rank==nprocs-1 ) {
       			MPI_Recv( &accessMat( surface, 0, 0 ), columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
       		} else {
       			MPI_Recv( &accessMat( surface, p_rows+1, 0 ), columns, MPI_FLOAT, rank+1, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
       			MPI_Recv( &accessMat( surface, 0, 0 ), columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
       		}
           ...
           /* 4.2.1. Update heat on active focal points */
   		    for( i=0; i<num_focal; i++ ) {
   		      ...
   		      if(x >= i_row - i_region){
   		        if(x < i_row - i_region + f_total ){
   		          accessMat( surface, x-i_row+i_region, y ) = focal[i].heat;
   						}
   					}
   		    }
   		    /* 4.2.3. Update surface values (skip borders) */
   		    ...
   		    /* 4.2.4. Compute the maximum residual difference (absolute value) */
   		        ...
   		        for( i=i_region; i<f_region; i++ ){
   		          if( global_residual >= THRESHOLD ) break;
   		          for( j=1; j<columns-1; j++ ){
   		            if ( accessMat( surfaceCopy, i, j ) - accessMat( surface, i, j )  >= THRESHOLD ) {
   		              global_residual = THRESHOLD;
   		              break;
   		            }
                 }
   		...
   		MPI_Allreduce( MPI_IN_PLACE, &global_residual, 1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD );
   		...
   		for( t=0; t<num_teams; t++ ) {
   			...
   			if ( teams[t].x-radius >= f_row) continue;
   			if ( teams[t].x+radius < i_row) continue;
               ...
               if ( i >= i_row )
                 if ( i < f_row )
   			           accessMat( surface, i - i_row + i_region, j ) = accessMat( surface, i - i_row + i_region, j ) * ( 1 - 0.25 ); // Team efficiency factor
   					}
   				}
   			}
   		}
       ...
       if(focal[i].x >= i_row ){
         if(focal[i].x < f_row ){
   		    aux[i] = accessMat( surface, (focal[i].x - i_row + i_region ), focal[i].y );
   			}
   		}
   	}

   	MPI_Reduce( aux, residualHeat, num_focal, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD );
