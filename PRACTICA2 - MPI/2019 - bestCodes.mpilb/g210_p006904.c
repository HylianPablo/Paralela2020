...
// ExecutionRequest[P:'AntooroCines.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 19:00:37
 ...
 */
	...
	int my_rows = rows / nprocs;
	int my_begin = rank * my_rows;
    int my_final = my_begin+my_rows;
	
	
	/* Reasignacion de halo + restantes */
	
	if(rank==0){	
		my_rows=my_rows+1;
		my_final=(my_begin+my_rows)-1;
	} else if( rank>=1 && rank<nprocs-1){
		my_begin=my_begin-1;	
		my_rows = my_rows + 2;
	}
	else{
		my_begin=my_begin-1;
		my_rows= my_rows+(rows%nprocs) +1;
		my_final=(my_begin+my_rows)-1;
	}
	...
	if( rank==0 ){
		...
		my_rows=my_rows+1; 
		if(rows%nprocs!=0){
			my_rows=my_rows+(rows%nprocs);
		}
	}else{
		my_rows=my_rows+2;
	}
	
    int my_final = my_begin+my_rows-1;
	*/
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			for( i=0; i<num_focal; i++ ) {
				...
				int x = focal[i].x;
				int y = focal[i].y;
				if (x-my_begin>=0 && x-my_begin<my_rows){
				//if (x>my_begin && x<my_final){
				//accessMat( surface, x, y ) = focal[i].heat;
					accessMat(surface,x-my_begin,y)=focal[i].heat;  
				}
			}
		...
		float *copia = surface;
			...
			if (rank<1){
                            MPI_Send(&(accessMat(surface,my_rows-2,0)),columns,MPI_FLOAT,rank+1,tag,MPI_COMM_WORLD);
                            MPI_Recv(&(accessMat(surface,my_rows-1,0)),columns,MPI_FLOAT,rank+1,tag,MPI_COMM_WORLD,&stat);
                        }else if (rank>=1 && rank<nprocs-1){
							MPI_Send(&(accessMat(surface,1,0)),columns,MPI_FLOAT,rank-1,tag,MPI_COMM_WORLD);
                            MPI_Send(&(accessMat(surface,my_rows-2,0)),columns,MPI_FLOAT,rank+1,tag,MPI_COMM_WORLD);
						
							MPI_Recv(&(accessMat(surface,0,0)),columns,MPI_FLOAT,rank-1,tag,MPI_COMM_WORLD,&stat);
                            MPI_Recv(&(accessMat(surface,my_rows-1,0)),columns,MPI_FLOAT,rank+1,tag,MPI_COMM_WORLD,&stat);
                        }else{
							MPI_Send(&(accessMat(surface,1,0)),columns,MPI_FLOAT,rank-1,tag,MPI_COMM_WORLD);
                            MPI_Recv(&(accessMat(surface,0,0)),columns,MPI_FLOAT,rank-1,tag,MPI_COMM_WORLD,&stat);
	  ...
	  gl = global_residual;
	  MPI_Allreduce( &gl, &global_residual,1, MPI_FLOAT,  MPI_MAX,MPI_COMM_WORLD);
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if (i-my_begin >=0 && i -my_begin < my_rows){
					float distance = sqrtf ( dx*dx + dy*dy );
					if ( distance <= radius ) {
						accessMat( surface, i-my_begin, j ) = accessMat( surface, i-my_begin, j ) * ( 0.75 ); // Team efficiency factor
					}
				}
				}
			}
...
float *temp = (float*)malloc( sizeof(float) * (size_t) num_focal );
//float temp[num_focal];
	for (i=0; i<num_focal; i++){
		int x = focal[i].x;
		temp[i]=0.0f;
		residualHeat[i]=0.0f;
		...
		if (x-my_begin>0 && x-my_begin<my_rows){
		//	printf("%i ",x);
		temp[i] = accessMat( surface, focal[i].x-my_begin, focal[i].y );
		//residualHeat[i]=accessMat( surface, focal[i].x , focal[i].y);
		}
	}

	MPI_Reduce( temp, residualHeat, num_focal, MPI_FLOAT,  MPI_MAX ,0,MPI_COMM_WORLD);
