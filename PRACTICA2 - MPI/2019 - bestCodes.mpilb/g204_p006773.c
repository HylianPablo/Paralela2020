// User: g204@62.42.23.7 
// ExecutionRequest[P:'extinguishingV2.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 17:30:31
...
int main(int argc, char *argv[]) {
	...
	if ( read_from_file ) {
		...
		if ( ok != 3 ) {
			fprintf(stderr,"-- Errone piece 938r in file: reading rows, columns, max_iter from file: %s\n", argv[1]);
			MPI_Abort( MPI_COMM_WORLD,  EXIT_FAILURE );
		}
 ...
 */
	...
	float *residualHeat2 = (float*)malloc( sizeof(float) * (size_t)num_focal );
	if ( residualHeat2 == NULL ) {
		fprintf(stderr,"-- Error allocating: residualHeat\n");
		MPI_Abort( MPI_COMM_WORLD,  EXIT_FAILURE );
	}

	for (i=0;i<num_focal;i++){
		residualHeat2[i]=0.0f;
		}


	int num_procs;
	MPI_Comm_size(MPI_COMM_WORLD, &num_procs);
	//int rank_rows = rows/num_procs;
	//printf("Procesos: %d rank: %d\n",num_procs, rank);
	int iter;
	...
	int ultima_fila;
	//int filas_libres;
    	int filas_proceso = (int) (rows / num_procs);
    	int libres = (rows % num_procs);

	
	
	
	//printf("FILAS_PROCESO; %d , LIBRES: %d \n", filas_proceso,libres);
	if (libres==0){
		if (rank==0){
			primera_fila= filas_proceso*rank;
			ultima_fila = primera_fila + filas_proceso;
			//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n FILASFINAL: %d ",primera_fila,ultima_fila,ultima_fila-primera_fila); 
		}else if(rank<num_procs-1){
			primera_fila = filas_proceso * rank - 1; 
			ultima_fila = primera_fila + filas_proceso + 1;
			//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n FILASFINAL: %d ",primera_fila,ultima_fila,ultima_fila-primera_fila);
			} else {
				primera_fila = filas_proceso * rank - 1; 
				ultima_fila = primera_fila + filas_proceso;
	...
	if(libres!=0){
		if (rank==0){
			...
			//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n FILASFINAL: %d ",primera_fila,ultima_fila,ultima_fila-primera_fila);
				...
				primera_fila = filas_proceso * rank - 1; 
				ultima_fila = primera_fila + filas_proceso+libres;
				//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n FILASFINAL: %d ",primera_fila,ultima_fila,ultima_fila-primera_fila);
				}
	...
	if(libres==0 && rank>0 && rank<num_procs-1){
		primera_fila = filas_proceso * rank - 1;
	      	ultima_fila = primera_fila + filas_proceso + 1;
		//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n",primera_fila,ultima_fila);
 	}

	if(libres==0 && rank==num_procs-1){
		primera_fila= filas_proceso * rank -1;
		ultima_fila = primera_fila + filas_proceso;
		//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n",primera_fila,ultima_fila);		
	}
	...
	if(libres!=0){
	    	filas_libres=filas_proceso+1;
		if(rank<libres && rank!=0){
			primera_fila = filas_libres * rank - 1; 
			ultima_fila = primera_fila + filas_libres + 1;
			//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n",primera_fila,ultima_fila); 
		} 

		if (libres!=0 && rank==0){
			primera_fila= filas_libres * rank;
			ultima_fila = primera_fila + filas_libres;
			//printf("PRIMERA FILA: %d ULTIMA FILA: %d \n FILASFINAL: %d ",primera_fila,ultima_fila,ultima_fila-primera_fila); 
		}

		if(rank>=libres && rank!=num_procs-1 ){
			primera_fila = filas_libres*libres + (rank-libres) *filas_proceso - 1;
			ultima_fila = primera_fila + filas_proceso + 1;
			//printf("PRIMERA FILA SIN AÑADIR: %d ULTIMA FILA: %d \n",primera_fila,ultima_fila); 
		} 
		if(rank>=libres && rank==num_procs-1){
			primera_fila = filas_libres*libres + (rank-libres) *filas_proceso - 1;
			ultima_fila = primera_fila + filas_proceso;
			//printf("PRIMERA FILA SIN AÑADIR PROCESO FINAL: %d ULTIMA FILA: %d \n",primera_fila,ultima_fila); 
	 	}
	}
	...
	*/
	...
	int numero_filas_final=(ultima_fila-primera_fila+1);

	//printf("NUMERO FILAS CADA UNA: %d",numero_filas_final);
	float *surface = (float *)malloc( sizeof(float) * (size_t)numero_filas_final * (size_t)columns );
	float *surfaceCopy = (float *)malloc( sizeof(float) * (size_t)numero_filas_final * (size_t)columns );
...
// MPI Version: Eliminate this conditional to start doing the work in parallel
	...
	for( i=0; i<numero_filas_final; i++ ){
		for( j=0; j<columns; j++ ){
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10 ; step++ )	{
			...
			for( i=0; i<num_focal; i++ ) {
				...
				int y = focal[i].y;
				//printf("FOCOS %d , %d ,%d \n",x,y,focal[i].heat);
				if(x>=primera_fila && x<=ultima_fila){
					//contador++;
					//printf("PRIMERA FILA: %d ULTIMA FILA: %d X: %d \n",x-primera_fila,ultima_fila-x,x);
					accessMat( surface, x-primera_fila, y ) = focal[i].heat;
					//printf("CUANTAS VECES ENTRA AQUI\n");	
				}

			}
			/* 4.2.2. Copy values of the surface in ancillary structure (WITHOUT Skip borders) */
			/*
			for( i=0; i<numero_filas_final; i++ ){
				for( j=0; j<columns; j++ ){
			...
			*/
			...
			surface = surfaceCopy;
			surfaceCopy = temp;	
			/*for( i=0; i<numero_filas_final && rank==0; i++ ){
					// printf("\n");
						for( j=0; j<columns; j++ ){
							//printf("%f  ",accessMat( surface, i, j ));
							//if(accessMat( surface, i, j )!=0){
								//printf("%f ",accessMat( surface, i, j ));
								//}
							}
				}
			...
			*/
			...
			// BORRAR o COMENTAR PARA PONER QUITAR BUCLE
			for( i=1; i<numero_filas_final-1; i++ ){
				for( j=1; j<columns-1; j++ ){
					...
					if ( num_deactivated ==num_focal && fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) > global_residual ) ) {
			...
			MPI_Status status;
			if(rank!=num_procs-1){
				//MANDAR LA PENULTIMA FILA QUE CONTIENE LOS DATOS RELEVANTES AL PROCESO SIGUIENTE 1
				//printf("rank 0 manda matriz\n");
				MPI_Isend(&surface[(numero_filas_final-2)*columns],columns,MPI_FLOAT,rank+1,100,MPI_COMM_WORLD,&request);

				//RECIBIR ULTIMA FILA DEL PROCESO 1
				//MPI_Recv(&surface[(numero_filas_final-1)*columns],columns,MPI_FLOAT,rank+1,100,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				
			}
			
			if(rank!=0){
				//MANDAR LA SEGUNDA FILA AL PROCESO ANTERIOR	
				MPI_Isend(&surface[1*columns],columns,MPI_FLOAT,rank-1,100,MPI_COMM_WORLD,&request);

				//RECIBIR LA PRIMERA FILA DEL PROCESO ANTERIOR
				//MPI_Recv(&surface[0],columns,MPI_FLOAT,rank-1,100,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			}
			...
			if(rank!=num_procs-1){
				//MANDAR LA PENULTIMA FILA QUE CONTIENE LOS DATOS RELEVANTES AL PROCESO SIGUIENTE 1
				//printf("rank 0 manda matriz\n");
				//MPI_Isend(&surface[(numero_filas_final-2)*columns],columns,MPI_FLOAT,rank+1,100,MPI_COMM_WORLD,&status);
			...
			if(rank!=0){
				//MANDAR LA SEGUNDA FILA AL PROCESO ANTERIOR	
				//MPI_Isend(&surface[1*columns],columns,MPI_FLOAT,rank-1,100,MPI_COMM_WORLD,&status);
			...
			/*if(rank>0 && rank<num_procs-1){
				//printf("rank 1 y 2 manda matriz\n");	
				//MANDAR SEGUNDA FILA AL PROCESO ANTERIOR
				MPI_Send(&surface[1*columns],columns,MPI_FLOAT,rank-1,100,MPI_COMM_WORLD);
				//MANDAR PENULTIMA FILA AL PROCESO SIGUIENTE
				MPI_Send(&surface[(numero_filas_final-2)*columns],columns,MPI_FLOAT,rank+1,100,MPI_COMM_WORLD);
				//RECIBIR LA PRIMERA FILA DEL PROCESO ANTERIOR
				MPI_Recv(&surface[0],columns,MPI_FLOAT,rank-1,100,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				//RECIBIR LA ULTIMA FILA DEL PROCESO SIGUIENTE
				MPI_Recv(&surface[(numero_filas_final-1)*columns],columns,MPI_FLOAT,rank+1,100,MPI_COMM_WORLD,MPI_STATUS_IGNORE);	
			}*/
...
/*
			if(rank==0){
				//RECIBIR EN LA ULTIMA FILA QUE ES LA QUE COMPARTE
				//printf("rank 0 recibe matriz\n");	
				MPI_Irecv(&surface[(numero_filas_final-1)*columns],columns,MPI_FLOAT,rank+1,tag,MPI_COMM_WORLD,&request2);
			}

			if(rank==num_procs-1){
				//RECIBIR EN LA PRIMERA FILA
				//printf("rank 3 recibe matriz\n");	
				MPI_Irecv(&surface[(0)*columns],columns,MPI_FLOAT,rank-1,tag,MPI_COMM_WORLD,&request2);
			}

			if(rank>0 && rank<num_procs-1){
				//printf("rank 2 y 3 recibe matriz\n");	
				//RECIBIR EN LA PRIMERA FILA
				MPI_Irecv(&surface[(0)*columns],columns,MPI_FLOAT,rank-1,tag,MPI_COMM_WORLD,&request2);
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( j=0; j<num_focal; j++ ) {
				...
				//if ( focal[j].x<primera_fila || focal[j].x >ultima_fila) continue; 
					...
					for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
						...
						if ( i >= primera_fila && i <= ultima_fila){
							...
							if ( distance <= radius*radius ) {
								accessMat( surface, i-primera_fila, j ) = accessMat( surface, i-primera_fila, j ) * ( 1 - 0.25 ); // Team efficiency factor
...
print_status( iter, rows, columns, surface, num_teams, teams, num_focal, focal, global_residual );
	...
	//printf("NUMERO DE FOCOS: %d\n",num_focal);
	//float heat=0.0f;
	// MPI Version: Store the results of residual heat on focal points
	//float *residualHeat2 = (float*)malloc( sizeof(float) * (size_t)num_focal )

	for (i=0; i<num_focal; i++){
		if( (focal[i].x-ultima_fila)<=0 && (focal[i].x-primera_fila)>=0 ){
			residualHeat[i]=0.0f;
			residualHeat[i] = accessMat( surface, focal[i].x-primera_fila, focal[i].y );
			
			//printf("calor %f \n",heat);
		}
		//printf("Calor: %f \n",accessMat( surface, focal[i].x-primera_fila, focal[i].y));
		

		
	}
	MPI_Allreduce(residualHeat,residualHeat2,num_focal,MPI_FLOAT,MPI_MAX,MPI_COMM_WORLD);
		//MPI_Allreduce(&global_residual,&max_residual,1,MPI_FLOAT,MPI_MAX,MPI_COMM_WORLD);		
	residualHeat=residualHeat2;
