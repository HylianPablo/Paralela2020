// User: g201@157.88.125.242 
// ExecutionRequest[P:'juejuejue.c',P:1,T:1,args:'',q:'mpilb'] 
// Apr 04 2019 20:03:25
 ...
 */
	...
	my_begin= rank*(rows/num_proc)+(rank < rows % num_proc ? rank : rows % num_proc);
...
/*int error=0;
	for(i=0;i<num_focal;i++)
		if(focal[i].x==0 || focal[i].y==0 || focal[i].x>=999 || focal[i].y>=999)
			...
			printf("error  %d \n",error);
*/
const int RADIUS_TYPE_1_SQUARED = 9;
const int RADIUS_TYPE_2_3_SQUARED = 81;
	float *surface = (float *)malloc( sizeof(float) * (size_t)(size+4) * (size_t)columns );
	float *surfaceCopy = (float *)malloc( sizeof(float) * (size_t)(size+4) * (size_t)columns );
	...
	int start=max_iter;
	for(i=0;i<num_focal;i++)
		if(focal[i].start<start)
			start=focal[i].start;
	for( iter=start; iter<max_iter; iter++ ) {
		...
		/* 4.2. Propagate heat (10 steps per each team movement) */
			...
			//printf("rank %d, size %d, my_begin %d my_begin+size %d \n",rank,size,my_begin,my_begin+size-1);
	...
	/*****************************************************************/
				...
				MPI_Request r4=MPI_REQUEST_NULL;
		for(int p=0;p<5;p++){
			...
			for( step=0; step<2; step++ )	{
				...
				for( i=0; i<num_focal; i++ ) {
					...
					int x = focal[i].x;
					int y = focal[i].y;
					if(my_begin-2<=x && x<my_begin+size+2){
					accessMat( surface, x-my_begin+2, y ) = focal[i].heat;
			...
			if(rank==0)
				...
				for( i=1+step; i<2+size+1-step; i++ )
					for( j=1; j<columns-1; j++ )
							accessMat( surface, i, j ) = (
								accessMat( surfaceCopy, i-1, j ) +
								accessMat( surfaceCopy, i+1, j ) +
			...
			if( num_deactivated == num_focal && step==0 && p==0)
			for( i=2; i<size+2; i++ )
				for( j=1; j<columns-1; j++ )
		...
		if(rank!=num_proc-1){
								MPI_Isend(&accessMat( surface, 2+size-2, 0 ), 2*columns, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD,&r1);
								MPI_Irecv(&accessMat( surface, 2+size, 0 ), 2*columns, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD,&r3);
								//MPI_Wait(&r1,MPI_STATUS_IGNORE);
								//MPI_Wait(&r3,MPI_STATUS_IGNORE);
		...
		if(rank!=0){
								MPI_Isend(&accessMat( surface, 2, 0 ), 2*columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD,&r2);
								MPI_Irecv(&accessMat( surface, 0, 0 ), 2*columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD,&r4);
								//MPI_Wait(&r4,MPI_STATUS_IGNORE);
								//MPI_Wait(&r2,MPI_STATUS_IGNORE);
						}
}
/******************************************************************************************************/
		...
		MPI_Iallreduce(&global_residual, &aux1, 1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD,&r9);
		}
		/* 4.3. Move teams */
		if(num_deactivated!=num_focal){
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-RADIUS_TYPE_1; i<=teams[t].x+RADIUS_TYPE_1; i++ ) {
				for( j=teams[t].y-RADIUS_TYPE_1; j<=teams[t].y+RADIUS_TYPE_1; j++ ) {
					...
					if ( (teams[t].x - i)*(teams[t].x - i) + (teams[t].y - j)*(teams[t].y - j) <= RADIUS_TYPE_1_SQUARED ) {
						if(my_begin-2<=i && i<my_begin+size+2){


							accessMat( surface, i-my_begin+2, j ) = accessMat( surface, i-my_begin+2, j ) * ( 0.75 );
						}
					}
				}
			}
		...
		if(num_deactivated==num_focal){
			...
			/*	if(rank==3 ){
								printf("iter %d   size %d my_begin %d rank %d\n",iter,size,my_begin,rank);
							for(i=0;i<size+4;i++){
								for(j=0;j<columns;j++){
	...
	for (i=0; i<num_focal; i++){
		if(my_begin<=focal[i].x && focal[i].x<my_begin+size && rank==0){
			residualHeat[i] = accessMat( surface, focal[i].x-my_begin+2, focal[i].y );
		}
		else if(my_begin<=focal[i].x && focal[i].x<my_begin+size && rank!=0){
			MPI_Send(&accessMat( surface, focal[i].x-my_begin+2, focal[i].y ) , 1, MPI_FLOAT, 0, i, MPI_COMM_WORLD);
		}
		else if(rank==0){
