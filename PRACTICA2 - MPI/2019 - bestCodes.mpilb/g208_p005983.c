 ...
 */
	...
	tamano=rows/procesos+(rank< rows % procesos ? 1 : 0); //calcula el tamano si es menor suma 1 si no suma 0 
	inicio= rank*(rows/procesos)+(rank < rows % procesos ? rank : rows % procesos);// incia en funcion de lo anterior


// MPI Version: Eliminate this conditional to start doing the work in parallel

	float *surface = (float *)malloc( sizeof(float) * (size_t)(tamano+2) * (size_t)columns );
	float *surfaceCopy = (float *)malloc( sizeof(float) * (size_t)(tamano+2) * (size_t)columns );
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		/* 4.2.1. Update heat on active focal points */
				...
				for( i=0; i<num_focal; i++ ) {
					...
					int x = focal[i].x;
					int y = focal[i].y;
					if(inicio-1<=x && x<=inicio+tamano){
					accessMat( surface, x-inicio+1, y ) = focal[i].heat;
		...
		if(rank!=procesos-1){
			MPI_Send(&accessMat( surface, tamano, 0 ), columns, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD);
			MPI_Recv(&accessMat( surface, tamano+1, 0 ), columns, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD,&info);
		}
		if(rank!=0){
			MPI_Send(&accessMat( surface, 1, 0 ), columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD);
			MPI_Recv(&accessMat( surface, 0, 0 ), columns, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD,&info);
		}
		...
		MPI_Reduce(&global_residual, &auxiliar, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
		
		if(rank==0){
			if( num_deactivated == num_focal && auxiliar < THRESHOLD ) {
				flag_stability=1;
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( distance <= radius ) {
						if(inicio<=i && i<=inicio+tamano)
							accessMat( surface, i-inicio+1, j ) = accessMat( surface, i-inicio+1, j ) * ( 1 - 0.25 );
					}
				}
			}
		}
	...
	for (i=0; i<num_focal; i++){
		if(inicio<=focal[i].x && focal[i].x<inicio+tamano && rank==0){
			residualHeat[i] = accessMat( surface, focal[i].x-inicio+1, focal[i].y );
			//envio++;
		}
		else if(inicio<=focal[i].x && focal[i].x<inicio+tamano && rank!=0){
			MPI_Send(&accessMat( surface, focal[i].x-inicio+1, focal[i].y ) , 1, MPI_FLOAT, 0, i, MPI_COMM_WORLD);
			
		}
		else if(rank==0){
			
			MPI_Recv(&residualHeat[i], 1, MPI_FLOAT, MPI_ANY_SOURCE, i, MPI_COMM_WORLD,&info);
