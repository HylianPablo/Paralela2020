...
*/
    ...
    for(i=0; i< 3; i++){
    	posiciones[i]=malloc((size_t)max_comida*sizeof(float));
    }
...
*/			//printf("Mi rank: %d, num_new_sources en food spot: %d\n", rank, num_new_sources);
        ...
        num_cells_movidas=0;
        Cell *celulas_a_mover = (Cell *) malloc(sizeof(Cell));        
        ...
        free( celulas_movidas_por_proceso );
        free( celulas_movidas_totales);
		...
		if ( auxiliar_step_new_cells > 0 ) {
			cells_chiquito = (Cell *)realloc( cells_chiquito, sizeof(Cell) * ((size_t)( my_num_cells) + (size_t)auxiliar_step_new_cells ));
            food_to_share = (float *)realloc(food_to_share, sizeof(float) * ((size_t)( my_num_cells) + (size_t)auxiliar_step_new_cells ));
